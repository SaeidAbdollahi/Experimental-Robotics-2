...
def planning(goal):
    ...
    while not rospy.is_shutdown():
        ...
        if act_s.is_preempt_requested():
            ...
            break
        elif err_pos < distance_threshold:
            change_state(2)
            ...
            feedback.actual_pose = pose_
            act_s.publish_feedback(feedback)
            if regions_['front'] < obstacle_threshold:
                change_state(1)
        elif state_ == 1:
            ...
            if regions_['front'] > free_way_threshold and math.fabs(err_yaw) < alignment_threshold:
                change_state(0)
        elif state_== 2:
            break
